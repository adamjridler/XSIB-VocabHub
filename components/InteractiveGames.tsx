...
export function InteractiveGames({ onBack, backgroundWords, onGameComplete, initialGameData }: { onBack: () => void; backgroundWords?: string[], onGameComplete?: () => void, initialGameData?: { game: string, configId: string | null } | null }) {
  const [selectedGame, setSelectedGame] = useState<'word-fall' | 'fill-blanks' | 'memory-match' | 'word-scramble' | 'word-search' | 'mini-crossword' | 'multiplayer-memory' | 'multiplayer-radial' | null>(null);
  ...
  const [radialOptionsCount, setRadialOptionsCount] = useState<4 | 6 | 8>(6);
  ...
  if (gameState === 'playing' && selectedGame) {
    if (gameWords.length === 0 && !(selectedGame === 'multiplayer-memory' && roomAction === 'join') && !(selectedGame === 'multiplayer-radial' && roomAction === 'join')) {
      return (
      ...
      <div ref={containerRef} className="fixed inset-0 w-full h-full flex flex-col bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-100 font-sans overflow-hidden">
        ...
        <div className="flex flex-col h-full w-full min-h-0 overflow-hidden bg-slate-900/90 backdrop-blur-2xl md:rounded-[2.5rem] border-0 md:border border-white/10 shadow-2xl text-slate-100 font-sans relative z-10 transition-colors">
          ...
          <main className="flex-1 min-h-0 w-full h-full relative p-0 pt-16 flex flex-col">
            ...
            ) : selectedGame === 'multiplayer-radial' ? (
              <MultiplayerRadialMatchGame words={gameWords} mode={multiplayerMode} optionsCount={radialOptionsCount} roomId={roomId} isHost={roomAction === 'create'} onGameOver={handleGameOver} />
            ) : (
  ...
  return (
    <div className="fixed inset-0 w-full h-full overflow-hidden flex flex-col bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-100 text-slate-900 font-sans">
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 relative z-10 custom-scrollbar">
          ...
          <div className="w-full max-w-[100rem] mx-auto flex flex-col xl:flex-row gap-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              ...
              <motion.div 
                whileHover={{ y: -6, scale: 1.02 }}
                transition={{ type: "spring", stiffness: 300, damping: 20 }}
                className="bg-white flex flex-col border-2 border-slate-200 hover:border-indigo-400 hover:shadow-xl hover:shadow-indigo-500/10 transition-all cursor-pointer rounded-2xl overflow-hidden group"
                onClick={() => { setSelectedGame('multiplayer-radial'); setGameState('setup'); }}
              >
                <div className="h-32 bg-gradient-to-br from-indigo-500 to-blue-600 p-4 pb-3 flex flex-col justify-end relative overflow-hidden">
                   <div className="absolute inset-0 opacity-30 flex justify-center items-center overflow-hidden">
                      <div className="w-20 h-20 rounded-full border-4 border-indigo-200/50 absolute scale-150 group-hover:scale-[2] transition-transform duration-1000 ease-out"></div>
                   </div>
                   <div className="flex justify-between items-end relative z-10 w-full gap-2 h-full">
                     <h3 className="text-xl font-extrabold text-white drop-shadow-sm leading-tight max-w-[75%] break-words">Radial Rush</h3>
                     <div className="bg-white/20 backdrop-blur-sm p-3 rounded-2xl flex-shrink-0 group-hover:scale-110 transition-transform duration-700">
                       <Timer className="w-6 h-6 text-white" />
                     </div>
                   </div>
                </div>
                <div className="p-4">
                  <p className="text-slate-600 text-xs sm:text-sm font-medium leading-relaxed">A fast-paced multiplayer game where you compete to snap the correct translation instantly!</p>
                </div>
              </motion.div>
            </div>
        ...
        {gameState === 'setup' && (
          <div className={`flex flex-col ${selectedGame === 'multiplayer-memory' || selectedGame === 'multiplayer-radial' ? '' : 'xl:flex-row'} gap-6 md:gap-8 h-full max-w-7xl mx-auto`}>
            ...
            <div className="flex flex-col lg:flex-row gap-6 md:gap-8">
              {/* Context / Filters */}
              {!( (selectedGame === 'multiplayer-memory' || selectedGame === 'multiplayer-radial') && roomAction === 'join') && (
              ...
              <div className="flex-1 space-y-4 md:space-y-6 lg:max-w-md flex flex-col justify-between">
                <div className="space-y-4 md:space-y-6">
                  {selectedGame !== 'memory-match' && selectedGame !== 'word-scramble' && selectedGame !== 'word-search' && selectedGame !== 'mini-crossword' && selectedGame !== 'multiplayer-memory' && selectedGame !== 'multiplayer-radial' && (
                  <div>
                ...
                {selectedGame === 'multiplayer-radial' && (
                  <div className="space-y-4">
                    ...
                    {roomAction === 'join' ? (
                       <div>
                         <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2 bg-slate-100/50 inline-block px-2 py-1 rounded-md">Room Code</h3>
                         <input type="text" value={roomId} onChange={(e) => setRoomId(e.target.value.toUpperCase().slice(0,6))} maxLength={6} placeholder="Enter 6-digit code" className="w-full text-center text-2xl font-black tracking-widest p-4 border-2 border-slate-200 rounded-xl focus:border-indigo-500 outline-none uppercase" />
                       </div>
                    ) : (
                    <>
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-2 md:gap-3">
                        <div 
                          onClick={() => setMultiplayerMode('translation')}
                          className={`cursor-pointer p-2 md:p-3 rounded-xl border-2 shadow-sm transition-all text-center font-bold text-xs md:text-sm ${multiplayerMode === 'translation' ? 'border-indigo-500 bg-indigo-50/80 text-indigo-700 shadow-indigo-500/20' : 'border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50 text-slate-600'}`}
                        >
                          Translations
                        </div>
                        <div 
                          onClick={() => setMultiplayerMode('definition')}
                          className={`cursor-pointer p-2 md:p-3 rounded-xl border-2 shadow-sm transition-all text-center font-bold text-xs md:text-sm ${multiplayerMode === 'definition' ? 'border-indigo-500 bg-indigo-50/80 text-indigo-700 shadow-indigo-500/20' : 'border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50 text-slate-600'}`}
                        >
                          Definitions
                        </div>
                      </div>
                    ...
                    <div>
                      <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2 bg-slate-100/50 inline-block px-2 py-1 rounded-md">3. Radial Options</h3>
                      <div className="grid grid-cols-3 gap-2 md:gap-3">
                        {[4, 6, 8].map(count => (
                          <div 
                            key={count}
                            onClick={() => setRadialOptionsCount(count as any)}
                            className={`cursor-pointer rounded-xl border-2 p-2 md:p-3 flex flex-col items-center gap-1 md:gap-2 transition-all group ${radialOptionsCount === count ? 'border-indigo-500 bg-indigo-50 text-indigo-700 shadow-sm' : 'border-slate-200 hover:border-indigo-300 hover:bg-slate-50 text-slate-600'}`}
                          >
                            <Timer className={`w-5 h-5 md:w-6 md:h-6 ${radialOptionsCount === count ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-500'} transition-colors`} />
                            <span className="font-bold text-xs md:text-sm text-center">{count} items</span>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div className="pt-2">
                  <div className="bg-slate-50 p-3 md:p-4 rounded-xl md:rounded-2xl border-2 border-slate-200/60 mb-3 md:mb-4 shadow-sm">
                    ...
                    <p className="text-xs text-slate-600 leading-snug font-medium space-y-1">
                      ...
                      {selectedGame === 'multiplayer-radial' && 'Join a room and match the correct word to the given definition in the center as fast as you can. First to tap wins!'}
                    </p>
                  </div>
                  <Button 
                    onClick={handleStartGame} 
                    disabled={filteredWords.length === 0 && !( (selectedGame === 'multiplayer-memory' || selectedGame === 'multiplayer-radial') && roomAction === 'join')}
                  ...
                  >
                    {(selectedGame === 'multiplayer-memory' || selectedGame === 'multiplayer-radial') && roomAction === 'join' ? 'Join Game' : (selectedGame === 'multiplayer-memory' || selectedGame === 'multiplayer-radial') ? 'Create Room' : 'Start Game'}
                  </Button>
                </div>
              </div>
            </div>
          ...
          {selectedGame !== 'multiplayer-memory' && selectedGame !== 'multiplayer-radial' && (
