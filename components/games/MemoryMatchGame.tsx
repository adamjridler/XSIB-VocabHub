        ...
        <div className="max-w-5xl w-full grid grid-cols-1 lg:grid-cols-12 gap-6 items-center mt-6">
          ...
          <div className="lg:col-span-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3">
            <div className="bg-slate-900/40 p-4 md:p-5 rounded-2xl border border-white/5 flex flex-col justify-center">
              ...
              <p className="text-xs font-medium text-slate-500 mt-1">{matchedIds.length} matches / {moves} moves</p>
            </div>

            <div className="bg-slate-900/40 p-4 md:p-5 rounded-2xl border border-white/5 flex flex-col justify-center">
              <div className="flex items-center gap-3 mb-1 text-slate-400">
                <Clock className="w-4 h-4 text-blue-400" />
                <span className="font-bold tracking-wide uppercase text-xs">Time</span>
              </div>
              <p className="text-3xl font-black text-white">{timePassed}s</p>
            </div>

            <LeaderboardForGame configId={`MemoryMatch-${previewTime}-${timeLimit}`} limit={4} />
          </div>
        </div>
      </motion.div>
    );
